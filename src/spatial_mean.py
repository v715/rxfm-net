...
class SpatialMean_CHAN(nn.Module):
    ...
    def __init__(
        self, input_shape, eps=1e-9, pytorch_order=True, return_power=False, **kwargs
    ):
        ...
        for idx, in_shape in enumerate(self.input_shape_nb_nc):
            ...
            coord_idx_tensor = torch.reshape(
                coord_idx_tensor, [in_shape] + [1] * (len(self.input_shape_nb_nc) - 1)
            )
            ...
            coord_idx_tensor = coord_idx_tensor.repeat(
                *(
                    [1]
                    + self.input_shape_nb_nc[:idx]
                    + self.input_shape_nb_nc[idx + 1 :]
                )
            )
            ...
            coord_idx_tensor = coord_idx_tensor.permute(
                *(
                    list(range(1, idx + 1))
                    + [0]
                    + list(range(idx + 1, len(self.input_shape_nb_nc)))
                )
            )

            self.coord_idx_list.append(torch.reshape(coord_idx_tensor, [-1]))
            # self.coord_idx_list.append(
            #  torch.reshape(torch.cast(coord_idx_tensor, tf.float32),[-1])
            # )

        print(
            "WARNING [spatial_mean]: pytorch reverses its axes because why not.",
