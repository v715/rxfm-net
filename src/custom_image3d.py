...
def create_transform(rx=0, ry=0, rz=0, tx=0, ty=0, tz=0):
    ...
    shift_matrix = np.array([[1, 0, 0, tx], [0, 1, 0, ty], [0, 0, 1, tz], [0, 0, 0, 1]])
...
def apply_transform_torch(
    x, transform_matrix, channel_axis=3, fill_mode="nearest", cval=0.0, order=1
):
...
class ImageTransformer(object):
    ...
    def random_transform(self, *args):
        ...
        if self.shear_range:
            sxy, sxz, syx, syz, szx, szy = np.random.uniform(
                -self.shear_range, self.shear_range, 6
            )
        ...
        if self.zoom_range[0] != 1 and self.zoom_range[1] != 1:
            ...
            zoom_matrix = np.array(
                [[zx, 0, 0, 0], [0, zy, 0, 0], [0, 0, zz, 0], [0, 0, 0, 1]]
            )
            if transform_matrix is None:
